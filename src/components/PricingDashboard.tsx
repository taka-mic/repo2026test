    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-violet-600 rounded-2xl p-6 text-white shadow-lg shadow-indigo-200">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="font-semibold text-indigo-100 text-sm">AI推奨 最適賃料（1K/1DK基準）</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white/10 rounded-xl p-3 text-center">
            ...
            <div className="font-bold">¥{pricing.minRent.toLocaleString()}</div>
          </div>
          <div className="bg-white/10 rounded-xl p-3 text-center">
            <div className="text-xs text-indigo-200 mb-0.5">上限</div>
            <div className="font-bold">¥{pricing.maxRent.toLocaleString()}</div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          <div>
            ...
            <div className="text-base font-bold text-slate-700">¥{pricing.comparison.areaAverage.toLocaleString()}</div>
          </div>
          <div>
            <div className="text-xs text-slate-400 mb-1">類似物件</div>
            <div className="text-base font-bold text-slate-700">¥{pricing.comparison.similarBuildings.toLocaleString()}</div>
          </div>
          <div>
            <div className="text-xs text-slate-400 mb-1">プレミアム率</div>
            <div className={`text-base font-bold ${pricing.comparison.premiumRate >= 0 ? "text-emerald-600" : "text-red-500"}`}>
      ...
      {/* Price Factors */}
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        <h3 className="font-bold text-slate-700 text-sm mb-4">価格決定要因</h3>
            ...
            <div key={i} className="flex items-center py-2 border-b border-slate-50 last:border-0">
              ...
              <div className="flex-1 mx-3">
                <div className="h-1.5 bg-slate-100 rounded-full relative overflow-hidden">
                  <div className="absolute inset-y-0 bg-indigo-200 rounded-full"
                    style={{
                      left: "0%",
                      width: "100%",
                    }}
